...
OUTPUT_LOCATION = "E:/Ankit_Live/ics_output_file/"
...
def process_asset_df(input_asset_df):
    ...
    if len(main_image_df):
        main_image_df_grouped = main_image_df.groupby('hash_key')[columns_to_include].apply(
            lambda group: group.to_dict('records')[-1] if not group.empty else {}).reset_index(name='json_container')
    else:
...
product_df["_scrape_metadata"] = product_df["_scrape_metadata"].apply(
    lambda x: {
        **x,
        "date_visited": x['date_visited'].replace('-27T',"-28T").strip() if len(x['date_visited']) == 24 else x['date_visited'][:-1] + ":00.000Z",
